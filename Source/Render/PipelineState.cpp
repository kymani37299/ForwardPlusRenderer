...
namespace GFX
{
    ...
    uint32_t GetHash(const PipelineState& pipelineState);

	const CompiledPipelineState& CompilePipelineState(const PipelineState& pipelineState)
	{
        uint32_t hash = GetHash(pipelineState);
    ...
    uint32_t GetHash(const PipelineState& pipelineState)
    {
        ...
        const uint8_t* bytes = reinterpret_cast<const uint8_t*>(&pipelineState);
        for (uint32_t i = 0; i < sizeof(PipelineState); i++)
        {
